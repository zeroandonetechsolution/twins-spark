(function () {
  ...
  function loadProducts() {
    try {
      const stored = localStorage.getItem('twins-spark-products');
      if (stored) {
  ...
  function initSmoke() {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
      ...
      particle.style.setProperty('--smoke-scale', String(Math.random() * 0.8 + 0.5));
      particle.style.setProperty('--smoke-rotate', `${Math.random() * 360}deg`);
      particle.style.setProperty('--smoke-duration', `${Math.random() * 15 + 30}s`);
      particle.style.setProperty('--smoke-delay', `${Math.random() * -40}s`);
